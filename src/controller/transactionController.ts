    ...
    try {
        ...
        if(notFoundMedicine.length > 0){
            return res.status(200).json({
                message: `There are medicine that not exists`
            })
        }
...
const readTransaction = async(req: Request, res: Response) => {
    try {
        ...
        /** mendapatkan seluruh data transaction sekaligus detail di tiap transaction nya */
        ...
        if(req.query.start_date && req.query.end_date) {
            allTransaction = allTransaction.filter(trans => 
                trans.order_date >= start_date && trans.order_date <= end_date
            )
        }
